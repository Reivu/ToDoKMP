...
class MongoDB {
    ...
    fun readActiveTask(): Flow<RequestState<List<ToDoTask>>> {
    ...
    suspend fun updateTask(task: ToDoTask) {
        realm?.write {
            try {
                val queryTask = query<ToDoTask>("_id == $0", task._id)
                    .first()
                    .find()
                queryTask?.let {
        ...
        realm?.write {
            try {
                val queryTask = query<ToDoTask>(query = "_id == $0", task._id)
                    .find()
                    .first()
                queryTask.apply { completed = taskCompleted }
            } catch (e: Exception) {
                println(e)
            }
        }
        ...
        realm?.write {
            try {
                val queryTask = query<ToDoTask>(query = "_id == $0", task._id)
                    .find()
                    .first()
                queryTask.apply { favorite = isFavorite }
            } catch (e: Exception) {
                println(e)
            }
        }
